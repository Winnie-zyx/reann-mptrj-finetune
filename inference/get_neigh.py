...
class Neigh_List(torch.nn.Module):
    ...
    def forward(self,period_table,coordinates,cell,mass):
        ...
        num_repeats_up = (num_repeats.data+1).detach() # ADD .data to adjust to pytorch-1.12.1, or you get RuntimeError: Cannot input a tensor that requires grad as a scalar argument in calculation of r1
        num_repeats_down = (-num_repeats.data).detach()
        num_repeats_up=num_repeats_up.requires_grad_(False)
        num_repeats_down=num_repeats_down.requires_grad_(False)
